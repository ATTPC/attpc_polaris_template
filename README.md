...
This is a template codespace for using the [attpc_spyral](https://github.com/ATTPC/Spyral)
analysis framework on Polaris, the supercomputer at the ALCF at ANL. It serves as a 
location from which one can supply the appropriate configuration controls and contains 
a script which can create a PBS job script and submit the script to PBS to run the 
analysis.

This template can be used in practice, or it can be used simply as a guide for how to 
setup a codespace to use attpc_spyral at Polaris.
...
This will load the appropriate Polaris modules, create a virtual environment, install 
...
Polaris work to run Spyral (you should still [read](https://docs.alcf.anl.gov/polaris/getting-started/) 
up on it though). To use `spyral_job.py`, you need to make sure you have your 
environment [active](#the-setup). You can then simply run the script as
...
Using `multiprocessing.set_start_method("dragon")` sets Dragon as the method by which 
...
In general, the only path you need to modify is the `LIBFABRIC_PATH`. This should be 
